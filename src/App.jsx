...
function App() {
  ...
  const [tempFiles, setTempFiles] = useState(
    () =>
      JSON.parse(localStorage.getItem("rawDataStorage")) || {
        lastWeek: null,
        thisWeek: null,
        lastMonth: null,
        thisMonth: null,
  ...
  const [reportFiles, setReportFiles] = useState(() => {
    ...
    return saved
      ? JSON.parse(saved)
      : { 1: Array(6).fill(null), 2: Array(6).fill(null) };
  });

  const [manualAdData, setManualAdData] = useState(
    () => JSON.parse(localStorage.getItem("manualAdData")) || null
  );

  const [activeTab, setActiveTab] = useState("weekly");
  const [memo, setMemo] = useState(
    () => JSON.parse(localStorage.getItem("dashboardMemo")) || {}
  );
  const [adHistory, setAdHistory] = useState(
    () => JSON.parse(localStorage.getItem("adHistory")) || {}
  );
  ...
  useEffect(
    () => localStorage.setItem("dashboardData", JSON.stringify(persistedData)),
    [persistedData]
  );
  useEffect(
    () => localStorage.setItem("dashboardMemo", JSON.stringify(memo)),
    [memo]
  );
  useEffect(
  ...
  useEffect(
    () => localStorage.setItem("rawDataStorage", JSON.stringify(tempFiles)),
    [tempFiles]
  );
  useEffect(
  ...
  useEffect(
    () => localStorage.setItem("manualAdData", JSON.stringify(manualAdData)),
    [manualAdData]
  );

  // [수정 2] 성과보고서 파일 상태 저장
  useEffect(
    () => localStorage.setItem("reportFiles", JSON.stringify(reportFiles)),
    [reportFiles]
  );
  ...
  const handlePaste = async (text) => {
    ...
    try {
      ...
      setTempFiles((p) => ({
        ...p,
        [pasteModal.target]: { data: d, name: "붙여넣은 데이터" },
      }));
      setPasteModal({ open: false, target: "" });
      alert("데이터가 입력되었습니다!");
    } catch (err) {
      alert("데이터 인식 실패: " + err.message);
    }
  };
  ...
  const moveThisMonthToLast = () => {
    if (!tempFiles.thisMonth)
      return alert("이동할 '이번달' 데이터가 없습니다.");
    if (!confirm("이번달 데이터를 지난달로 이동하시겠습니까?")) return;
    setTempFiles((prev) => ({
      ...prev,
      lastMonth: prev.thisMonth,
      thisMonth: null,
    }));
    alert("이동 완료!");
  };
  ...
  const runAnalysis = () => {
    ...
    if (tempFiles.thisWeek) {
      newData.weekly = processWeeklyAnalysis(
        tempFiles.thisWeek.data,
        tempFiles.lastWeek?.data || []
      );
    }
    if (tempFiles.thisMonth) {
      ...
      if (tempFiles.lastMonth)
        newData.report = processPerformanceReport(
          tempFiles.thisMonth.data,
          last
        );
    }
  ...
  const handleDownloadReport = () => {
    if (!tempFiles.thisMonth || !tempFiles.lastMonth)
      return alert(
        "월말 정산 리포트 생성에는 '이번달'과 '지난달' 데이터가 필요합니다."
      );
    try {
      const processedCurrent = processWeeklyAnalysis(
        tempFiles.thisMonth.data,
        tempFiles.lastMonth.data
      );
      ...
      combined = weekly.map((wItem) => {
        ...
        if (mItem) {
          const unionTypes = new Set([
            ...wItem.adEligibleTypes,
            ...mItem.adEligibleTypes,
          ]);
  ...
  const TABS = [
    ...
    { id: "external_analysis", label: "📈 분석" },
  ];

  const currentData =
    activeTab === "monthly" ? persistedData.monthly : persistedData.weekly;

  return (
    <div
      className={`min-h-screen font-sans transition-colors duration-300 ${
        isDark ? "dark bg-gray-900 text-gray-100" : "bg-gray-100 text-gray-800"
      } pb-20`}
    >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-xl w-[600px] border dark:border-gray-700">
            <div className="flex justify-between mb-4">
              ...
              <button
                onClick={() => setPasteModal({ open: false, target: "" })}
              >
                <X />
              </button>
            </div>
            <textarea
              ...
              placeholder="복사(Ctrl+C) 후 붙여넣기(Ctrl+V)"
            ></textarea>
            <button
              onClick={() =>
                handlePaste(document.getElementById("pasteArea").value)
              }
              className="w-full bg-indigo-600 dark:bg-indigo-700 text-white py-3 rounded font-bold hover:bg-indigo-700 transition"
            >
              입력하기
            </button>
          </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                onClick={resetAll}
                className="bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 px-3 py-2 rounded font-bold text-xs flex items-center gap-1 hover:bg-red-100 dark:hover:bg-red-900/30 transition"
              >
              ...
              <div className="flex bg-gray-100 dark:bg-gray-700 p-1 rounded-lg">
                {TABS.map((t) => (
                  <button
                    ...
                    onClick={() => {
                      // 분석 탭이면 새 창으로 링크 열기
                      if (t.id === "external_analysis") {
                        window.open(
                          "https://hongcafe-fe.vercel.app/login",
                          "_blank"
                        );
                      } else {
                        // 그 외에는 기존처럼 탭 전환
                    ...
                    className={`px-2 py-2 text-sm font-bold rounded-md transition ${
                      activeTab === t.id
          ...
          {activeTab === "weekly" && (
            <div className="flex gap-4 items-stretch">
              <UploadBox
                label="1. 지난주 (선택)"
                fileData={tempFiles.lastWeek}
                onUpload={(e) => handleUpload(e, "lastWeek")}
                onPaste={() =>
                  setPasteModal({ open: true, target: "lastWeek" })
                }
              />
              <div className="flex flex-col justify-center items-center px-2">
                <button
                  onClick={moveThisToLast}
              ...
              <UploadBox
                label="2. 이번주 (필수)"
                fileData={tempFiles.thisWeek}
                onUpload={(e) => handleUpload(e, "thisWeek")}
                onPaste={() =>
                  setPasteModal({ open: true, target: "thisWeek" })
                }
                color="blue"
              />
              <div className="flex flex-col gap-2 flex-1 min-w-[200px] justify-center">
                <button
                  onClick={runAnalysis}
                  className="bg-indigo-600 dark:bg-indigo-700 text-white rounded-lg py-4 hover:bg-indigo-700 dark:hover:bg-indigo-600 font-bold flex items-center justify-center gap-2 transition shadow-md w-full"
                >
          ...
          {activeTab === "monthly" && (
            <div className="flex gap-4 items-stretch">
              <UploadBox
                label="1. 비교 데이터 (과거)"
                fileData={tempFiles.lastMonth}
                onUpload={(e) => handleUpload(e, "lastMonth")}
                onPaste={() =>
                  setPasteModal({ open: true, target: "lastMonth" })
                }
              />
              <div className="flex flex-col justify-center items-center px-2">
                <button
                  onClick={moveThisMonthToLast}
                  className="text-gray-400 dark:text-gray-500 hover:text-indigo-600 transition"
                >
                  <ArrowRightCircle size={24} />
                </button>
              ...
              <UploadBox
                label="2. 기준 데이터 (최신)"
                fileData={tempFiles.thisMonth}
                onUpload={(e) => handleUpload(e, "thisMonth")}
                onPaste={() =>
                  setPasteModal({ open: true, target: "thisMonth" })
                }
                color="purple"
              />
              <div className="flex flex-col gap-2 flex-1 min-w-[200px]">
                <button
                  onClick={runAnalysis}
                  className="flex-1 bg-indigo-600 dark:bg-indigo-700 text-white rounded-lg hover:bg-indigo-700 dark:hover:bg-indigo-600 font-bold flex items-center justify-center gap-2 transition shadow-md w-full"
                >
                  <RefreshCw size={20} className="animate-spin-slow" />{" "}
                  <span>분석 실행</span>
                </button>
                <div className="flex items-center gap-2 bg-green-50 dark:bg-green-900/20 p-2 rounded-lg border border-green-100 dark:border-green-800">
                  ...
                  <button
                    ...
                    className="flex-1 bg-green-600 dark:bg-green-700 text-white rounded-md py-2 hover:bg-green-700 font-bold flex items-center justify-center gap-2 transition shadow-sm text-sm"
                  >
      ...
      <div className="max-w-full mx-auto p-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm min-h-[600px] p-6 border border-gray-200 dark:border-gray-700 transition-colors">
          ...
          {activeTab === "weekly" &&
            (persistedData.weekly ? (
              <DashboardView
                data={persistedData.weekly}
                memo={memo}
                setMemo={setMemo}
                isDark={isDark}
              />
          ...
          {activeTab === "monthly" &&
            (persistedData.monthly ? (
              <DashboardView
                data={persistedData.monthly}
                memo={memo}
                setMemo={setMemo}
                isMonthly
                isDark={isDark}
          ...
          {activeTab === "revenue" &&
            (persistedData.revSummary ? (
              <RevenuePage
                summary={persistedData.revSummary}
                memo={memo}
                setMemo={setMemo}
              />
            ) : (
          ...
          {activeTab === "report" && (
            <SixMonthReport
              filesStore={reportFiles}
              setFilesStore={setReportFiles}
            />
          )}
        </div>
      </div>
