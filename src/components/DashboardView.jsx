    ...
    return (
        <ResponsiveContainer width="100%" height={height}>
          <BarChart data={dataset} margin={{top:20, right:30, left:20, bottom:5}}>
              ...
              <XAxis dataKey="nick" tick={{fontSize:14, fill: axisColor}} interval={0} axisLine={{stroke: axisColor}} tickLine={{stroke: axisColor}} />
              <YAxis tickFormatter={(val) => chartType==='revenue' ? `${val/10000}만` : `${Math.floor(val/3600)}시간`} tick={{fontSize:12, fill: axisColor}} axisLine={{stroke: axisColor}} tickLine={{stroke: axisColor}} />
              <Tooltip 
                  formatter={(val, name) => [chartType==='revenue' ? fmt(val)+'원' : fmtTime(val), name]}
                  contentStyle={{ backgroundColor: isDark ? '#1f2937' : '#fff', borderColor: isDark ? '#374151' : '#ccc', color: isDark ? '#f3f4f6' : '#000' }}
                  itemStyle={{ color: isDark ? '#f3f4f6' : '#000' }}
                  labelStyle={{ color: isDark ? '#9ca3af' : '#000', marginBottom: '0.5rem' }}
                  cursor={{fill: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)'}}
              />
              <Legend wrapperStyle={{fontSize: '14px', color: axisColor}}/>
              <Bar dataKey={chartType==='revenue'?'prevRev':'prevTime'} fill={isDark ? "#4b5563" : "#e5e7eb"} name={isMonthly ? "지난달" : "지난주"} radius={[4,4,0,0]} />
              <Bar dataKey={chartType==='revenue'?'curRev':'curTime'} fill={chartType==='revenue'?"#6366f1":"#10b981"} name={isMonthly ? "이번달" : "이번주"} radius={[4,4,0,0]} />
          </BarChart>
        </ResponsiveContainer>
    );
};
...
const DashboardView = ({ data, memo, setMemo, isMonthly, isDark }) => {
  ...
  const [thresholds, setThresholds] = useState({ missed: 10, unanswered: 5, minTime: 30, revDrop: 10 });

  const handleOpenMsg = (row) => { setSelectedMsgCounselor(row); setIsMsgModalOpen(true); };
  const handleSettingChange = (key, val) => { setThresholds(prev => ({ ...prev, [key]: Number(val) })); };
  
  const renderDelta = (val, type) => {
    if (!val || val === 0) return null;
    const isPos = val > 0;
    const color = isPos ? 'text-blue-600 dark:text-blue-400' : 'text-red-600 dark:text-red-400';
    const sign = isPos ? '+' : '-';
  ...
  const recalculatedData = useMemo(() => {
    return data.map(row => {
      ...
      const prevRev = row.prevRev || 0; const curRev = row.curRev || 0;
  ...
  const filteredData = useMemo(() => {
      ...
      if (filterCode && filterCode !== 'true') {
        try { const filterFn = new Function('item', `return ${filterCode}`); result = result.filter(item => filterFn(item)); } catch (e) { console.error(e); }
      }
      result.sort((a, b) => {
          let valA, valB, order = 'desc';
          if (aiSortConfig) { valA = a[aiSortConfig.key] || 0; valB = b[aiSortConfig.key] || 0; order = aiSortConfig.order; } 
          else { valA = chartType === 'revenue' ? a.curRev : a.curTime; valB = chartType === 'revenue' ? b.curRev : b.curTime; }
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-xl p-6 w-96 shadow-2xl animate-fade-in-up border dark:border-gray-700 transition-colors">
            <div className="flex justify-between items-center mb-4 border-b dark:border-gray-700 pb-2"><h3 className="font-bold text-lg flex items-center gap-2 text-gray-900 dark:text-gray-100"><Settings size={20}/> 이슈 기준 설정</h3><button onClick={() => setShowSettings(false)} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200"><X size={24}/></button></div>
            <div className="space-y-4">
              {['missed', 'unanswered', 'minTime', 'revDrop'].map((field, i) => (
                  <div key={field}><label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{['📞 C: 부재중', '✍️ D: 미답변 후기', '⏰ A: 최소 접속시간', '📉 B: 매출 하락'][i]}</label><input type="number" className="w-full border dark:border-gray-600 p-2 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-500 outline-none" value={thresholds[field]} onChange={(e)=>handleSettingChange(field, e.target.value)} /></div>
              ))}
            </div>
            <div className="mt-6 flex justify-end"><button onClick={() => setShowSettings(false)} className="bg-indigo-600 dark:bg-indigo-700 text-white px-4 py-2 rounded-lg font-bold hover:bg-indigo-700 dark:hover:bg-indigo-600 flex items-center gap-2 shadow-sm"><Save size={16}/> 저장</button></div>
          </div>
        </div>
      )}
      ...
      <div className="mb-8 p-4 border dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 shadow-sm transition-colors">
        <div className="flex justify-between items-center mb-4">
            <div className="flex gap-2 bg-gray-100 dark:bg-gray-700 p-1 rounded-lg">
                <button onClick={()=>setChartType('revenue')} className={`px-4 py-2 text-sm font-bold rounded-md transition ${chartType==='revenue'?'bg-white dark:bg-gray-600 shadow text-indigo-600 dark:text-indigo-300':'text-gray-500 dark:text-gray-400'}`}>💰 정산금액</button>
                <button onClick={()=>setChartType('time')} className={`px-4 py-2 text-sm font-bold rounded-md transition ${chartType==='time'?'bg-white dark:bg-gray-600 shadow text-green-600 dark:text-green-400':'text-gray-500 dark:text-gray-400'}`}>⏰ 접속시간</button>
            </div>
            <div className="flex items-center gap-2">
                {aiSortConfig && (<span className="text-xs font-bold bg-indigo-100 dark:bg-indigo-900 text-indigo-700 dark:text-indigo-200 px-2 py-1 rounded">🤖 {aiSortConfig.key} 정렬 중</span>)}
                <span className="text-xs font-bold text-gray-500 dark:text-gray-400">총 {filteredData.length}명</span>
                <button onClick={()=>setShowModal(true)} className="text-sm text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 flex items-center gap-1 font-bold border dark:border-gray-600 px-3 py-1.5 rounded hover:bg-gray-50 dark:hover:bg-gray-700 transition"><Maximize2 size={16}/> 전체보기</button>
            </div>
        </div>
        <div className="h-72 w-full"><ChartComponent dataset={top10Data} chartType={chartType} isMonthly={isMonthly} isDark={isDark} /></div>
      </div>
      ...
      {showModal && (
          <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 md:p-10">
              <div className="bg-white dark:bg-gray-800 rounded-xl w-full h-full max-w-7xl p-6 flex flex-col shadow-2xl border dark:border-gray-700 transition-colors">
                  <div className="flex justify-between items-center mb-4 border-b dark:border-gray-700 pb-2"><h3 className="font-bold text-xl flex items-center gap-2 text-gray-900 dark:text-gray-100">{chartType==='revenue' ? <span className="text-indigo-600 dark:text-indigo-400">💰 정산금액</span> : <span className="text-green-600 dark:text-green-400">⏰ 접속시간</span>} 전체 비교</h3><button onClick={()=>setShowModal(false)} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition text-gray-500 dark:text-gray-400"><X size={28}/></button></div>
                  <div className="flex-1 overflow-x-auto overflow-y-hidden pb-4 scrollbar-thin scrollbar-thumb-gray-400 dark:scrollbar-thumb-gray-600 scrollbar-track-gray-200 dark:scrollbar-track-gray-800">
                      <div style={{ width: `${Math.max(100, filteredData.length * 60)}px`, height: '100%' }}><ChartComponent dataset={filteredData} chartType={chartType} isMonthly={isMonthly} height="100%" isDark={isDark} /></div>
                  </div>
              </div>
          </div>
      )}
      ...
      <div className="overflow-x-auto rounded-lg border dark:border-gray-700 shadow-sm">
        <table className="w-full text-sm text-center whitespace-nowrap border-collapse table-fixed">
          <thead className="bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 font-bold uppercase sticky top-0 z-10 shadow-sm">
            <tr>
              <th className="p-3 w-28 sticky left-0 bg-gray-100 dark:bg-gray-800 border-r dark:border-gray-700 z-20">닉네임</th>
              <th className="p-3 w-24">카테고리</th>
              <th className="p-3 w-24">단계</th>
              <th className="p-3 w-20">레벨</th>
              <th className="p-3 bg-blue-50 dark:bg-blue-900/20 w-32">접속시간</th>
              <th className="p-3 bg-blue-50 dark:bg-blue-900/20 w-24">증감률</th>
              <th className="p-3 bg-blue-50 dark:bg-blue-900/20 w-32">정산금액</th>
              <th className="p-3 bg-blue-50 dark:bg-blue-900/20 w-24">증감률</th>
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-800 dark:text-gray-300">
            {filteredData.map((row, idx) => {
              const timeColor = row.timeRate < 0 ? 'text-red-600 dark:text-red-400' : 'text-blue-600 dark:text-blue-400';
              const revColor = row.revRate < 0 ? 'text-red-600 dark:text-red-400' : 'text-blue-600 dark:text-blue-400';
              let rowClass = 'hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors';
              let stickyClass = 'bg-white dark:bg-gray-800';
              if (row.status === 'blind') { rowClass = 'bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/30'; stickyClass = 'bg-red-50 dark:bg-red-900/20'; } 
              else if (row.status === 'new') { rowClass = 'bg-yellow-50 dark:bg-yellow-900/20 hover:bg-yellow-100 dark:hover:bg-yellow-900/30'; stickyClass = 'bg-yellow-50 dark:bg-yellow-900/20'; }

              return (
                  <tr key={idx} className={rowClass}>
                    <td className={`p-3 font-bold sticky left-0 border-r dark:border-gray-700 z-10 text-gray-800 dark:text-gray-100 ${stickyClass}`}>{row.nick}</td>
                    ...
                    <td className={`p-3 font-bold ${row.unanswered > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-400 dark:text-gray-600'}`}>{row.unanswered}</td>
                    <td className="p-3">{row.curMissed}</td>
                    <td className="p-3 text-left">
                      <div className="flex flex-col gap-1 items-start">
                        {row.remarks!=='-' && <span className="text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-700 border dark:border-gray-600 px-2 py-0.5 rounded text-xs font-medium">{row.remarks}</span>}
                        {row.issues.map(code => <span key={code} className="px-2 py-0.5 rounded text-xs font-bold border flex items-center gap-1 bg-yellow-50 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-400 border-yellow-200 dark:border-yellow-800">{ISSUE_LABELS[code] || code}</span>)}
                      </div>
                    </td>
                    <td className="p-3"><button onClick={() => handleOpenMsg(row)} className={`px-3 py-1.5 rounded text-xs font-bold transition-all border flex items-center justify-center gap-1 w-full ${(row.issues && row.issues.length > 0) || row.status === 'blind' || row.status === 'new' ? 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 border-purple-200 dark:border-purple-800 hover:bg-purple-200 dark:hover:bg-purple-900/50' : 'bg-white dark:bg-gray-700 text-gray-400 dark:text-gray-400 border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-600'}`}><MessageCircle size={14} />{(row.issues?.length > 0) ? '생성' : '메시지'}</button></td>
                    <td className="p-3"><input className="border dark:border-gray-600 rounded px-2 py-1.5 w-full bg-white dark:bg-gray-700 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-shadow text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500" placeholder="메모..." value={memo[row.nick]||''} onChange={e=>setMemo({...memo,[row.nick]:e.target.value})} /></td>
                  </tr>
              );
            })}
            {filteredData.length === 0 && (<tr><td colSpan="13" className="p-10 text-gray-400 dark:text-gray-500">검색 조건에 맞는 상담사가 없습니다.</td></tr>)}
          </tbody>
        </table>
      </div>
      <MessageModal isOpen={isMsgModalOpen} onClose={() => setIsMsgModalOpen(false)} counselor={selectedMsgCounselor} />
    </div>
  );
};
